...
def validate_filters(filters):
	...
	if (filters.to_date < filters.year_start_date) or (filters.to_date > filters.year_end_date):
		...
		filters.to_date = filters.year_end_date
	# if not filters.is_group:
	# 	filters.is_group = 1

def get_data(filters):
	# conditions = "1=1"
	#
	# if filters.get('is_group'):
	# 	conditions += f" AND is_group = '{filters.get('is_group')}'"

	accounts = frappe.db.sql(
		f"""SELECT name, account_number, parent_account, account_name, root_type, is_group, report_type, lft, rgt
	...
	min_lft, max_rgt = frappe.db.sql(
		...
		where company=%s AND is_group=%s""",
		(filters.company,filters.is_group),
	)[0]
...
def get_rootwise_opening_balances(filters, report_type):
	...
	if filters.get('is_group'):
		additional_conditions += f" AND tabAccount.is_group = '{filters.get('is_group')}' "
